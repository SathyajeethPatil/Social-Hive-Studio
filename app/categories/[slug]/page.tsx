    ...
    <PageShell>
      <main className="pb-24 pt-24">
        <section className="relative min-h-[56vh] overflow-hidden px-6 pt-20">
          {hero ? <Image src={hero} alt={category.name} fill priority className="object-cover" /> : null}
          ...
          <div className="relative mx-auto flex min-h-[48vh] max-w-7xl flex-col justify-end pb-12">
            <Link href="/categories" className="mb-6 inline-flex w-fit items-center gap-2 rounded-full bg-white/70 px-4 py-2 text-sm font-bold backdrop-blur">
              <ArrowLeft className="h-4 w-4" /> Back to Categories
            </Link>
            <p className="font-heading text-xs font-bold uppercase tracking-[0.24em] text-hivePink">Category</p>
            <h1 className="mt-3 max-w-4xl font-heading text-5xl font-extrabold md:text-7xl">{category.name}</h1>
          </div>
        </section>
